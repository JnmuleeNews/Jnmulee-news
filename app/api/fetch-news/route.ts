...
async function fetchArticlePage(
  ...
  for (
    const pattern of commonContainers
  ) {
    const match =
      html.match(pattern);
      ...
      try {
          ...
          try {
            ...
            if (
              ...
              errors.push(
                `${
                  source.name ||
                  source.id
                }: duplicate check failed: ${duplicateError.message}`
              );
            ...
            errors.push(
              `${
                source.name ||
                source.id
              }: article processing failed: ${message}`
            );
          }
        }
      } catch (error) {
        ...
        errors.push(
          `${
            source.name ||
            source.id
          }: source processing failed: ${message}`
        );
      }
    }
